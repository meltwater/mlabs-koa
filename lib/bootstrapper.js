...
const getLoggerOptions = ({config, logFilters = {}}) => {
  ...
  const {
    ...
    version = defaultTo(config.get('pkg:version'), config.get('LOG_VERSION')),
    service = defaultService,
    system = defaultSystem,
    env,
    outputMode = defaultTo('short', config.get('LOG_OUTPUT_MODE')),
    filter = defaultTo(null, config.get('LOG_FILTER')),
    base = {},
    ...logConfig
  ...
  const getDevLogConfig = () => {
    if (filter && !has(filter, logFilters)) {
      throw new Error(`Log output filter ${filter} not found`)
    }

    const outputFilter = filter ? logFilters[filter] : null

    return {outputFilter, outputMode}
  }
