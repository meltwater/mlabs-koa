...
const initialize = ({config, dependencies, startServer}) => {
  ...
  try {
    ...
    const createHealthMonitor = dependencies.resolve('createHealthMonitor')
    dependencies.registerValue({healthMonitor: createHealthMonitor(dependencies)})
    ...
    const onStart = () => {
      ...
      start().then(() => {
        startServer({config, log, app, healthMonitor, dependencies})
      }).catch(err => {
...
const createBootstrap = (createDependencies, startServer) => (err, config) => {
  ...
  try {
    const {env, ...logConfig} = defaultTo({}, config.get('log'))
