...
const getLoggerOptions = ({config, logFilters = {}}) => {
  ...
  const logBase = {
    ...
    version: defaultTo(version, config.get('LOG_VERSION')),
    ...base
  }

  const getDevLogConfig = () => {
    const filterName = defaultTo(filter, config.get('LOG_FILTER'))
    ...
    return {
      outputFilter,
      outputMode: defaultTo(outputMode, config.get('LOG_OUTPUT_MODE'))
    }
  }
