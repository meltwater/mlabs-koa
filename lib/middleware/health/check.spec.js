...
test('runs checks', t => {
  const { ctx, next, nextPromise, log, checks } = t.context
  td.when(ctx.request.accepts('json')).thenReturn(true)
  const result = check({log, checks: t.context.checks})(ctx, next)
  t.is(ctx.status, 202)
...
test('runs checks from context', t => {
  ...
  const result = check({log, checks: [doNotCall]})(ctx, next)
  t.is(ctx.status, 202)
  t.is(result, nextPromise)
  td.verify(checks[0]())
  td.verify(checks[1]())
...
test('runs checks and logs error', t => {
  ...
  const result = check({log, checks: [...checks, willFail]})(ctx, next)
  t.is(ctx.status, 202)
  t.is(result, nextPromise)
  td.verify(checks[0]())
  td.verify(checks[1]())
