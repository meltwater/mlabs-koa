...
export default ({ config, dependencies, startServer, watcher, logger }) => {
  ...
  try {
    ...
    const onFalseStart = exitOnFalseStart
      ...
      : err => { lifeLog.fatal({ err }, 'Startup: Fail') }
    ...
    watcher.once('ready', () => {
      watcherReady = true
      if (watcherClosed) watcher.close()
    })
    if (watcherReady) watcher.close()
    watcherClosed = true
    ...
    const onUnhandledRejection = exitOnUnhandledRejection
      ...
      : err => { lifeLog.fatal({ err }, 'Unhandled Rejection') }
